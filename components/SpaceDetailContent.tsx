...
export default function SpaceDetailContent({ space }: { space: Space }) {
  ...
  const title = t(`spaces.${space.type}`, space.title);
      ...
      <div className="grid gap-8 p-5 sm:p-6 md:grid-cols-[1.45fr_1fr] md:p-8">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-black text-[#073B3A] sm:text-3xl">
            Un espace prêt pour vos activités professionnelles
          </h2>
          <p className="mt-4 text-base leading-8 text-[#102A2A]/75">
            {space.detailedDescription}
          </p>

          <h3 className="mt-8 text-xl font-black text-[#073B3A]">{labels.usage}</h3>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <InfoGroup title={labels.equipment} items={space.equipment} />
            <InfoGroup title={labels.benefits} items={space.benefits} />
          </div>
        </div>
        ...
        <aside className="space-y-5 rounded-[1.75rem] bg-[#F4FAF9] p-5 sm:p-6">
          <div className="rounded-[1.5rem] bg-white p-5 shadow-lg shadow-[#00A6A6]/10 ring-1 ring-[#00A6A6]/10">
            ...
            <h2 className="mt-2 text-2xl font-black text-[#073B3A]">{space.category}</h2>
            <p className="mt-4 text-sm leading-7 text-[#102A2A]/68">
              Réservation sur demande avec accompagnement IPMES selon le format, la durée et les
              besoins techniques.
            </p>
          </div>
          ...
          <div className="rounded-[1.5rem] bg-white p-5 shadow-sm ring-1 ring-[#00A6A6]/10">
            ...
            <p className="mt-2 text-2xl font-black text-[#073B3A]">{space.price}</p>
          </div>

          <WhatsAppButton
            label={space.ctaLabel}
            message={`Bonjour IPMES, je souhaite réserver: ${title}.`}
          />

          <Link
            href={`/espaces?lang=${language}`}
